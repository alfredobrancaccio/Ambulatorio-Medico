...
def _valida_visita(medico_id, paziente_id, esame_id):
    ...
    esame = app_db.query_one(
        "SELECT id, nome, reparto_id FROM esami WHERE id = ?",
        (esame_id,),
    )
    if not esame:
...
def seg_reparti_modifica(id):
    ...
    if request.method == "POST":
        ...
        if not nome:
            ...
            app_db.execute(
                "UPDATE reparti SET nome = ? WHERE id = ?",
                (nome, id),
            )
...
def seg_medici_nuovo():
    if request.method == "POST":
        ...
        try:
            reparto_id = int(request.form["reparto_id"])
        except (ValueError, KeyError):
            flash("[ERRORE] Reparto non valido.")
        else:
            if not nome or not cognome:
                flash("[ERRORE] Nome e cognome sono obbligatori.")
            else:
...
def seg_esami_modifica(id):
    esame = app_db.query_one(
        "SELECT e.id, e.nome, e.reparto_id, r.nome AS reparto"
        " FROM esami e JOIN reparti r ON r.id = e.reparto_id WHERE e.id = ?",
        (id,),
    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not nome:
                ...
                app_db.execute(
                    "UPDATE esami SET nome=?, reparto_id=? WHERE id=?",
                    (nome, reparto_id, id),
                )
...
def seg_visite_nuovo():
    ...
    return render_template(
        ...
        esami=app_db.query(
            "SELECT e.id, e.nome, e.reparto_id FROM esami e ORDER BY e.nome"
        ),
    )
...
def med_dashboard(id):
    ...
    prescrizioni = app_db.query(
        _SQL_PRESCRIZIONI_FULL +
        " WHERE pr.medico_id = ? ORDER BY pr.data_emissione DESC",
        (id,),
    )

    n_referti = app_db.query_one(
        "SELECT COUNT(*) FROM referti rf"
        " JOIN visite v ON v.id = rf.visita_id WHERE v.medico_id = ?",
        (id,),
    )["COUNT(*)"]
    ...
    if request.method == "POST":
        try:
            ...
            visita = app_db.query_one(
                "SELECT id, medico_id FROM visite WHERE id = ?",
                (visita_id,),
            )
